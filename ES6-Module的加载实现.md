...
1. [浏览器加载](https://github.com/LbhFront-end/About-ES6/blob/master/ES6-Module的加载实现.md#浏览器加载)
2. [ES6 模块与 CommonJS 模块的差异](https://github.com/LbhFront-end/About-ES6/blob/master/ES6-Module的加载实现.md#es6-模块与-commonjs-模块的差异)
3. [Node 加载](https://github.com/LbhFront-end/About-ES6/blob/master/ES6-Module的加载实现.md#node-加载)
4. [循环加载](https://github.com/LbhFront-end/About-ES6/blob/master/ES6-Module的加载实现.md#循环加载)
5. [ES6 模块的转码](https://github.com/LbhFront-end/About-ES6/blob/master/ES6-Module的加载实现.md#es6-模块的转码)
...
如果脚本的体积很大，下载和执行的时间就会很长，因为造成浏览器堵塞，用户会感觉到浏览器卡死，没有任何响应，这显然是不好的体验，所以浏览器允许脚本异步加载。下面是两种异步加载的语法。
...
第二个差异是因为 CommonJS 加载的一个对象（即 `module.exports` 属性），该对象只有在脚本运行完才会生成，而 ES6 模块不是对象，它的对外接口只是一个静态定义，在代码静态解析阶段就会生成，
