...
与`y`修饰符相匹配，ES6的正则实例对象多了`sticky`属性，表示是否设置了`y`修饰符
...
ES6为正则表达式新增了`flags`模式，会返回正则表达式的修饰符
...
正则表达式中，点（`.`）是一个特殊字符，代表任意的单个字符，但是有两个例外。一个是四个字节的UTF - 16字符，这个可以用`u`修饰符解决；另外一个是行终止符（line terminator character）

所谓的行终止符，就是该字符表示一行的终结，一下四个字符属于“行终止符”
...
但是很多似乎还我们希望匹配的是任意单个字符，这时就有一种变通的写法
...
这种解决方法毕竟不太符合直觉，ES2018引入了`s`修饰符，使得`.`可以匹配单个字符

```
/foo.bar/s.test('foo\nbar') // true
```

这个被称为`dotAll`模式，即点（dot）代表一切字符，所以，正则表达式还引入了一个`dotAll`属性，返回一个布尔值，表示该正则表达式是否处在dotAll 模式
...
“先行断言”指的是，`x`只有在`y`前面才匹配，必须写成 `/x(?=y)/`。比如，只匹配百分号之前的数字，要写成`/\d+(?=%)/`。“先否定断言”指的是，`x`只有不在`y`前面才匹配，必须写成`/x(?!y/`。比如，只匹配不在百分号之前的数字，要写成`/\d+(?!%)/`。 
