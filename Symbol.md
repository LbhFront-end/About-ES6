...
由于每一个Symbol值都是不相等的，这就意味着Symbol可以作为标识符，用于对象的属性名，就能保证不会出现同名的属性。这对于一个对象由多个模块构成的情况非常有用，能防止某一个键被不小心改写或者覆盖
...
function getComplement(color){
    switch(color){
        ...
        case COLOR_GREEN:
        	return COLOR_RED;
        default:
        	throw new Rrror('Undefined color');
    }
}
```
...
function getArea(shape, options){
    ...
    switch(shape){
        case 'Triangle': // 魔术字符串
        area = .5 * options.width * options.heightl
        break;
        /.../
    }
    return area;
