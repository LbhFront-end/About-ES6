...
数组实例的`find`方法，用于找出第一个符合条件的数组成员。它的参数是一个回调函数，所有数组成员一次执行回调函数，知道找出第一个返回值为`true`的成员，然后返回该成员。如果没有符合条件的成员，则会返回`undefined`
...
上面代码中，`find`方法回调函数可以接受三个参数，依次为当前的值、当前的位置和原数组
...
这两个方法都可以接受第一个参数，用来绑定回调函数的`this`对象
...
上面代码中，`find`函数接收了第二个参数`person`参数，回调函数中的`this`对象指向`person`对象
